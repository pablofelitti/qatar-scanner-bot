...
```bash
qatar-scanner-bot$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
qatar-scanner-bot$ sam local start-api
qatar-scanner-bot$ curl http://localhost:3000/
```
...
```bash
qatar-scanner-bot$ sam logs -n HelloWorldFunction --stack-name qatar-scanner-bot --tail
```
...
```bash
qatar-scanner-bot$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name qatar-scanner-bot
```
